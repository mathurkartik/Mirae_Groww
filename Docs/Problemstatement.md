...
- **Home (Sovereign Explorer)**: Discovery landing page with category-based fund grouping, Top 5 Active table, NFO module, and Watchlist.
- **Category Listing**: Filterable and sortable list of all schemes within a category, with "Market Sentiment" banner.
- **Fund Detail (Analytics Dashboard)**: Deep-dive fund dashboard with investment objective, key metrics (Expense Ratio, AUM, Exit Load, Min SIP), interactive NAV chart, fully functional interactive Returns Calculator, risk profile, asset and sector allocation charts, holdings table, and peer comparison.
- **Atelier Advisor**: A floating AI insight widget ("Active Insight") accessible from any screen for factual Q&A. Features dark emerald header, responsive conversational bubbles, and inline citations.
- **Dual Layout Architecture**: Global layout (sidebar + inline nav) for discovery pages; Fund Detail layout (full-width nav + specialized sidebar) with smooth scroll-spying for analytics pages.
